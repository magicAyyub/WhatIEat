    ...
    >
      <Select.Trigger ref={triggerRef}>
        ...
        <TextInput
          ...
          className="w-[256px] h-12 px-3 rounded-2xl flex-row items-center bg-surface text-foreground text-base/5 shadow-md shadow-black/5"
          onFocus={() => {
            setIsFocused(true);
          ...
          {US_STATES.filter((state) =>
            ...
            <React.Fragment key={state.value}>
              ...
              {index < filteredArray.length - 1 && (
                <View className="h-px bg-divider mx-3" />
              )}
            </React.Fragment>
          ))}
          {US_STATES.filter((state) =>
